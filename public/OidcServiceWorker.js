  ...
  return response => {
    return response.json().then(tokens => {
      ...
      const secureTokens = {
        ...
        refresh_token:
          'REFRESH_TOKEN_SECURED_BY_OIDC_SERVICE_WORKER_' + configurationName,
      };
  ...
  if (numberDatabase > 0) {
    const maPromesse = new Promise((resolve, reject) => {
      const response = originalRequest.text().then(actualBody => {
        if (
          actualBody.includes('REFRESH_TOKEN_SECURED_BY_OIDC_SERVICE_WORKER')
        ) {
          let newBody = actualBody;
          for (let i = 0; i < numberDatabase; i++) {
            const currentDb = currentDatabases[i];
            const key =
              'REFRESH_TOKEN_SECURED_BY_OIDC_SERVICE_WORKER_' +
...
addEventListener('message', event => {
  ...
  switch (data.type) {
    ...
    case 'getAccessTokenPayload':
      const accessTokenPayload = extractAccessTokenPayload(
        currentDatabase.tokens.access_token
      );
